...
CREATE TABLE IF NOT EXISTS `levantamentos`.`elevadores` (
  ...
  `data_criacao` TIMESTAMP NULL,
  PRIMARY KEY (`id`, `piso_id`, `piso_edificio_id`),
...
CREATE TABLE IF NOT EXISTS `levantamentos`.`tipo_sistemas` (
  ...
  `fonte_energia` VARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `levantamentos`.`espaco` (
  ...
  PRIMARY KEY (`id`, `edificio_id`, `piso_id`, `ocupacao_id`, `iluminacao_id`, `envidracados_id`, `sistemas_id`),
...
CREATE TABLE IF NOT EXISTS `levantamentos`.`foto` (
  ...
  `elevadores_id` INT NULL,
  `espaco_id` INT NULL,
  `tipo_sistemas_id` INT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `fk_foto_edificio1_idx` (`edificio_id` ASC) VISIBLE,
  INDEX `fk_foto_elevadores1_idx` (`elevadores_id` ASC) VISIBLE,
  INDEX `fk_foto_espaco1_idx` (`espaco_id` ASC) VISIBLE,
  INDEX `fk_foto_tipo_sistemas1_idx` (`tipo_sistemas_id` ASC) VISIBLE,
  CONSTRAINT `fk_foto_edificio1`
    FOREIGN KEY (`edificio_id`)
    REFERENCES `levantamentos`.`edificio` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_foto_elevadores1`
    FOREIGN KEY (`elevadores_id`)
    REFERENCES `levantamentos`.`elevadores` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_foto_espaco1`
    FOREIGN KEY (`espaco_id`)
    REFERENCES `levantamentos`.`espaco` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_foto_tipo_sistemas1`
    FOREIGN KEY (`tipo_sistemas_id`)
    REFERENCES `levantamentos`.`tipo_sistemas` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `levantamentos`.`edificio` (
  ...
  `iluminacao_id` INT NULL,
  `proposta_id` VARCHAR(10) NOT NULL,
  PRIMARY KEY (`id`, `proposta_id`),
  INDEX `fk_edificio_iluminacao1_idx` (`iluminacao_id` ASC) VISIBLE,
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `fk_edificio_proposta1_idx` (`proposta_id` ASC) VISIBLE,
  CONSTRAINT `fk_edificio_iluminacao1`
    FOREIGN KEY (`iluminacao_id`)
    REFERENCES `levantamentos`.`iluminacao` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_edificio_proposta1`
